...
const navigation = [
  { name: 'My Appointments', icon: Calendar, id: 'appointments' },
  { name: 'Calendar View', icon: CalendarDays, id: 'calendar' },
  { name: 'My Patients', icon: Users, id: 'patients' },
  { name: 'My Profile', icon: User, id: 'profile' },
];

export function DoctorLayout({ children, currentPage, onNavigate, currentDoctor, onLogout }: DoctorLayoutProps) {
        ...
        <header className="sticky top-0 z-30 h-16 bg-white border-b border-gray-200">
          <div className="h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 ml-auto">
              ...
              <NotificationCenter userId={currentDoctor.id} userType="doctor" />
                ...
                <DropdownMenuTrigger className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                  <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="text-sm font-medium text-blue-700">
                      {currentDoctor.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                    </span>
                  </div>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-medium">{currentDoctor.name}</p>
