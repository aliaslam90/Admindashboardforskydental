...
export function Services() {
  ...
  const handleSave = () => {
    ...
    const exists = editingService.id && services.find(s => s.id === editingService.id);
    const payload = {
      category: editingService.category,
      name: editingService.name,
      duration_minutes: editingService.duration,
      active_status: editingService.active,
    };
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} className="bg-[rgb(203,255,143)] hover:bg-[#AEEF5A]">
              Save Service
            </Button>
          </DialogFooter>
        </DialogContent>
